...
// **List Businesses**
// **List Businesses**
// **List Businesses (Fuzzy Search + Trigrams)**
const listBusinesses = async (req, res) => {
    try {
        const { query: text = "", // search text
        longitude, latitude, } = req.query;
...
WHERE 
  ...
  AND (
    ...
    `;
        ...
        const businessesRaw = await prisma.$queryRawUnsafe(rawQuery, minLat, maxLat, minLng, maxLng, searchText);
        // ----------------------------------------------------------------
        ...
        const businessesWithDistance = businessesRaw
            .map((b) => {
            const distanceInMeters = (0, geolib_1.getDistance)({ latitude: userLatitude, longitude: userLongitude }, { latitude: b.longitude, longitude: b.latitude } // note your "swapped" naming
            );
        ...
        // ----------------------------------------------------------------
        const matchingKeywords = await prisma.$queryRawUnsafe(`
      SELECT t."name"
      FROM "Tag" t
